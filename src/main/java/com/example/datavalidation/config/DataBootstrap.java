...
public class DataBootstrap
{
    ...
    @Bean
    CommandLineRunner loadReferenceData(CurrencyRepository currencyRepository, CountryRepository countryRepository, BankIdentifierRepository bankIdentifierRepository)
    {
        return args -> {
            ...
            if (countryRepository.count() == 0)
            {
                ...
                countryRepository.save(new Country("Germany", "DE", "DEU", "+49", "Berlin", eur, "^DE\\d{20}$"));
                countryRepository.save(new Country("United Kingdom", "GB", "GBR", "+44", "London", gbp, "^GB\\d{2}[A-Z]{4}\\d{14}$"));
                countryRepository.save(new Country("United States", "US", "USA", "+1", "Washington", usd, null));
                countryRepository.save(new Country("Bulgaria", "BG", "BGR", "+359", "Sofia", bgn, "^BG\\d{2}[A-Z]{4}\\d{6}[A-Z0-9]{8}$"));
            }

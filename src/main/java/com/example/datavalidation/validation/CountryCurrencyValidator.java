...
@Component
public class CountryCurrencyValidator implements ConstraintValidator<CountryCurrencySupported, DataEntryRequest> {
    ...
    public boolean isValid(DataEntryRequest value, ConstraintValidatorContext context) {
        ...
        return countryDirectoryService.isCurrencyAllowedForCountry(country, currency)
                || buildViolation(context, country);
    }

    private boolean buildViolation(ConstraintValidatorContext context, String country) {
        context.disableDefaultConstraintViolation();
        context.buildConstraintViolationWithTemplate(
                        "Currency is not supported for country: " + country)
